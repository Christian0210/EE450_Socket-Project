 ...
 * Function: input()
 * ---------------------------------------------------------------------------------------------
 * Read and preprocess the CS course information from the file.
 */
 ...
 * Function: main(void)
 * ---------------------------------------------------------------------------------------------
 * Main function of the serverCS. Deal with the CS course request.
 */

int main(void)
{
	cout<<"The ServerCS is up and running using UDP on port "<<SERVERCSU<<".\n";
	...
	while (true) {
		string info = recvUDP(SERVERCSU);
		string ccd = info.substr(0, info.find(','));
		string ctg = info.substr(info.find(',') + 1);

		cout<<"The ServerCS received a request from the Main Server about the "<<out[ctg]<<" of "<<ccd<<".\n";
